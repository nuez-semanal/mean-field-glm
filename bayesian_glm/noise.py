...
class NoiseComputer(AuxiliaryFunctions):
    ...
    def cavity_observations( self, k ):
        ...
        if self.model.logl == "Logistic":
            return self.indicator(np.sqrt(self.model.snr)*cavity_projections-self.model.noise)
        else:
            return np.sqrt(self.model.snr)*cavity_projections-self.model.noise

    def compute_theta( self, k, j, cavity = True ):
        if cavity:
            fitted_values = self.cavity_fitted_values(k,j)
            observations = self.cavity_observations(k)
        else:
            fitted_values = np.dot(self.model.data,self.model.posterior[:j,:].transpose())
    ...
    def compute_noise_non_cavity( self, k, j = 100 ):
        theta = self.compute_theta( k, j, cavity = False )
        return np.mean(theta) / np.sqrt(self.hq)