...
if compute_2:
    ...
    for i, kappa in enumerate(kappa_values):
        ...
        print("\n***** Starting simulation for kappa = ",kappa," *****\n")
        for n in range(n_iter):
            model = ModelGLM(p=1000, n=int(1000/kappa), log_likelihood="Logistic", signal="Normal", prior="Normal", gamma=1.0, sigma=1.0)
            model.draw_sample()
            results[i*n_iter+n,0] = kappa
            results[i * n_iter + n, 1] = 1.0
            results[i * n_iter + n, 2], results[i * n_iter + n, 3], results[i * n_iter + n, 4]  = model.compute_order_parameters()
...
if compute_4:
    ...
    file_name = "mcmc_order_parameters_vs_kappa_lin_mismatch"
